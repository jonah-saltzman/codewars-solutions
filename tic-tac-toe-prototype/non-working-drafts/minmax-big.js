		...
		.map((position, index, board) => {
			if (!position) {
				...
				return {
					...
					score: minMax(newBoard, 0, false, _PLAYER, _OPPONENT, Number.NEGATIVE_INFINITY, Number.POSITIVE_INFINITY),
				}
			}
		})
		.filter((board) => board)
...
function minMax(board, depth, isMaximizer, _PLAYER, _OPPONENT, alpha, beta) {
	...
	if (isMaximizer) {
        ...
        for (let i = 0; i < nextBoards.length; i++) {
            maxScore = Math.max(maxScore, minMax(nextBoards[i], depth + 1, !isMaximizer, _PLAYER, _OPPONENT, alpha, beta))
        ...
        for (let i = 0; i < nextBoards.length; i++) {
            minScore = Math.min(minScore, minMax(nextBoards[i], depth + 1, !isMaximizer, _PLAYER, _OPPONENT, alpha, beta))
...
console.log(minMaxDriver(testBoard, 1))
